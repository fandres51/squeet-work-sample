...
export default function AddCardScreen(props) {
  ...
  const [titleTextbox, setTitleTextbox] = React.useState('');
  const [showTitlePrompt, setShowTitlePrompt] = React.useState(false);
  const [notes, setNotes] = React.useState('Tap to add notes');
  const [notesTextbox, setNotesTextbox] = React.useState('');
  ...
  const [image, setImage] = React.useState('https://squeet-cards.s3.amazonaws.com/default.png');
  ...
  async function add() {
    try {
      ...
      await axios.post(apiBase.apiBase + "decks/" + route.params.group.deck_id + "/addCard", { title, image: imageLink, notes, link }, { headers: await authHeader() });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomButtonContainer}>
        <TouchableOpacity onPress={add}>
          <Image style={[styles.addButton, { opacity: 1.0 }]} source={ImageDB.AddToDeck} />
        </TouchableOpacity>
      </View>
    </View>
  );
